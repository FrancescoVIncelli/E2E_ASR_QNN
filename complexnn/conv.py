...
#from layers.convolutional import _Conv
...
import numpy as np
import sys
#sys.path.append("C:/Users/vince/Desktop/MSc_Artificial_Intelligence_And_Robotics_Sapienza/MLSP/Project/complexnn")
from .init import *
...
class QuaternionConv(tf.keras.layers.Layer):
    ...
    def __init__(self, rank,
        ...
        self.data_format = tf.keras.backend.image_data_format() #(data_format)
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(
            shape = tuple(self.kernel_shape[:-1] + (self.kernel_shape[-1]*4,)),#tuple((self.kernel_shape[0], self.kernel_shape[1], 4*self.kernel_shape[2])), #self.kernel_shape,#self.kernel_size + (input_dim, self.filters), #self.kernel_shape,
    ...
    def call(self, inputs):
        ...
        cat_kernels_4_r = tf.keras.backend.concatenate([f_r, -f_i, -f_j, -f_k], axis=-2)
        cat_kernels_4_i = tf.keras.backend.concatenate([f_i, f_r, -f_k, f_j], axis=-2)
        cat_kernels_4_j = tf.keras.backend.concatenate([f_j, f_k, f_r, -f_i], axis=-2)
        cat_kernels_4_k = tf.keras.backend.concatenate([f_k, -f_j, f_i, f_r], axis=-2)
        cat_kernels_4_quaternion = tf.keras.backend.concatenate([cat_kernels_4_r, cat_kernels_4_i, cat_kernels_4_j, cat_kernels_4_k], axis=-1)
...
def sanitizedInitGet(init):
    if   init in ["sqrt_init"]:
        ...
        return tf.keras.initializers.get(init)
