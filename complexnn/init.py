...
#from tf.keras.utils.generic_utils import (serialize_keras_object, deserialize_keras_object)
...
class qconv_init(tf.keras.initializers.Initializer):
	...
	def __call__(self, shape, dtype=None):
		...
		fan_in, fan_out = shape[0], shape[1] #tf.keras.initializers._compute_fans(
...
class qdense_init(tf.keras.initializers.Initializer):
	# The standard complex initialization using
	# either the He or the Glorot criterion.
...
class sqrt_init(tf.keras.initializers.Initializer):
	def __call__(self, shape, dtype=None):
		return tf.keras.backend.constant(1 / tf.keras.backend.sqrt(2), shape=shape, dtype=dtype)
